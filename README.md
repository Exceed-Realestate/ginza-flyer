# 銀座セールスギャラリー — 開業チラシ（B5）

Exceed Real Estate の銀座セールスギャラリー（2026年10月18日開業）の
B5チラシ・レイアウト検討用ページ。

**Live:** https://exceed-realestate.github.io/ginza-flyer/

| ページ | 内容 |
|---|---|
| `index.html` | Version 1〜3 を1ページに |
| `version-1.html` | **Version 1（採用）** — 見出しは空の左上 |
| `versions-2-3.html` | Version 2（下寄せ）／ 3（左柱） |

サイズは B5 片面 182 × 257 mm。入稿時は塗り足し3mm（188 × 263 mm）。

## ページ上部のセレクタ

- **背景画像 ／ Background** — ヒーロー画像を切り替える。全バージョンに同時反映
- **レイアウト ／ Layout** — Version 1〜3 の表示切替（すべて表示も可）
- **フェード色 ／ Fade** — 帯の色6種（ネイビー／チャコール／ディープサンド／ディープティール／墨／ブロンズ）
- **EXCEED社章**（チェック）— 左上のマークの有無
- **主見出し ／ Headline** — 見出し6案の差し替え。文字サイズも案ごとに連動する
- **副見出し ／ Sub** — 副見出し6案
- **リード文 ／ Lead** — 本文6案
- **デベロッパーロゴ**（チェック）— 4社を社名テキスト⇄白ヌキロゴで切替
- **地図**（チェック）— 銀座コティビルの略図を予約欄に差し込む

> 2026-08-18: 縦組み案（旧 Version 2）は却下。以降 旧3→Version 2、旧4→Version 3 に繰り上げ。

目標は **開業の認知 ＋ LINE友だち追加**。予約獲得ではないため、CTAは「LINE公式アカウント、開設しました。」。

どちらの選択も `localStorage` に保存されるので、再訪時も同じ状態で開く。印刷時はセレクタ自体が消える（`@media print`）。

**主見出しの案**（`HEADLINES` 配列）:

| # | 主見出し | 賭けているもの |
|---|---|---|
| 1 | ドバイが、銀座に来ます。 | ニュース性。最短で一読で分かる |
| 2 | ドバイを、"比べて"選べる場所ができました。 | 差別化。既定値 |
| 5 | 見てから、決める。 | 最短。買い方の姿勢 |
| 6 | 銀座で、ドバイを歩く。 | 体験。歩けることを言う |
| 7 | 一日だけの説明会は、もう終わりにしませんか。 | カテゴリ攻撃。常設であること |
| 8 | 模型に、触れます。 | 感覚。最も具体的 |

**背景画像を追加するには** `index.html` 内の `BACKGROUNDS` 配列に1行足すだけ:

```js
const BACKGROUNDS = [
  { id:'ginza-vanishing-point',
    name:'GINZA VANISHING POINT ／ 消失点のドバイ',
    file:'assets/hero-v4-daylight.png' },
  // { id:'新しいID', name:'表示名', file:'assets/新しい画像.png' },
];
```

## ⚠️ 校了前の未確定事項

- ショールームの**階数**
- 予約用**電話番号**、**LINE ID**、**予約URL**
- 日本側の**法人名**
- ヒーロー画像は 687×1024 で**印刷解像度に不足**（要再生成）
- 「日本初」表示の**合理的根拠**（景品表示法）
- デベロッパー4社の**ロゴ使用許諾**
- ブルジュ・ハリファの**商業利用**（Emaar）

赤い「【　】未確定」タグは校正用。印刷前に削除すること。
